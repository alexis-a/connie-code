...
parts = [""]
invFilePrefix = "inv_scn_mbs_osi_"
invFileSufix = ".fits"
mskFileSufix = ".fits"
type = os.getenv('TYPE','')
#===================================
...
if len(sys.argv) <= 1:
	print
	print ">>>>>>> python extractInv.py <base directory>"
	print "<base directory> : directory where all the intermiate products of a set of images are stored"
	print "Extract objects from images (inv_scn_mbs_osi_*.fits) in the folder <base directory>/inv"
	print "Creates a list of images that were processed (listIEXT_*.dat) in <base directory>/iext/"
	print "The output root catalog is saved in <base directory>/iext/icatalog"
...
extDir =  os.path.join(baseDir,"iext")
outputDir = os.path.join(extDir,"icatalog")
...
invPath = os.path.join(baseDir,"inv")

invImagesPath = os.path.join(invPath,"images")
fileList = os.listdir(invImagesPath)
fileTimeList = [(os.path.join(invImagesPath,file),os.stat(os.path.join(invImagesPath,file)).st_mtime) 
				for file in fileList if file.startswith(invFilePrefix) and file.endswith(invFileSufix)]
...
for i in range(0,len(parts)):
	...
	#outfile name
	outFileName.append(os.path.join(outputDir,"icatalog_" + type + "_" + idInteval + part + ".root"))
