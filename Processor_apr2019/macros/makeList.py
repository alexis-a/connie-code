...
idLabel = "runID_" 
type = os.getenv('TYPE','')
skipFilename = "../macros/skipImages/all_runs_25ago2018.skip"
#===================================
...
#List of runIDs to skip.     #-A.A.-A. 5 Dec 2017
skipRunIDs = []
if os.path.isfile(skipFilename):
        print"\nSkipping runIDs in " + skipFilename + ":"
        with open(skipFilename) as file:
            #skipRunIDs = [line.strip() for line in file]
            for line in file: 
                skipRunIDs.append(line.strip())
else:
        print"\nNo Skip Images file given or found. Not skipping runIDs."

for  i in range (0,len(skipRunIDs)):
     print str(skipRunIDs[i])
...
outBaseDir = os.path.join(outDir,type + "_" + sufix)   # added "type" to base dir name A.A.-A. nov 2017
	...
	for file in partsList:
		...
		if indexRunID == -1:
			print "the name of the file: " + file + " is not supported by this script."
                        ...
                        # get imageID with reg-exp A.A.-A. Aug 2016
			# imageID = int(file[indexRunID + 9 :indexRunID + 9 + 5])  
                        pattern = re.compile(idLabel+"(\d*)_(\d*)(.*)" + part + fileSufix)
                        runum = pattern.search(file).group(1)
                        runid = pattern.search(file).group(2)
                        imgID = idLabel + runum + "_" + runid

			if int(runid) >= initID and int(runid) <= finID:
				if  not(imgID in skipRunIDs):   # skip imgs in skipFile  A.A.-A. 05dec2017
					#preguntar si tiene partes
					finalPartsList.append(file)
					outFile.write(file + "\n")
					counts = counts + 1
					print str(counts) + " " + file
				else:                          # skip imgs in skipFile  A.A.-A. 05dec2017
					print "file " + imgID + " part " + part + " skipped"
