...
int copyStructure(const string inFile, const char *outF, vector<int> &singleHdu){
    ...
    for (unsigned int eI=0; eI<nUseHdu; ++eI)  /* Main loop through each extension */
    {
        ...
        if (hdutype != IMAGE_HDU || naxis == 0 || totpix == 0){
            ...
            /* (Guillermo) create output image with size as specified in the global constants*/
            naxes[0] = (xActB[1] - xActB[0] + 1) + (xActA[1] - xActA[0] + 1);
            naxes[1] = yProc[1] - yProc[0] + 1;
...
int computeImage(const string inFile, const char *outF, vector<int> &singleHdu){
    ...
    for (unsigned int eI=0; eI<nUseHdu; ++eI)  /* Main loop through each extension */
    {
        ...
        {/* left side */
            ...
            {
                long fpixel[2]={xOverB[0],yProc[0]};
                long lpixel[2]={xOverB[1],yProc[1]};
                computeOvscMean(infptr, fpixel, lpixel, ovrscMeanR);
            }
            
            {
                long fpixel[2]={xActB[0],yProc[0]};
                long lpixel[2]={xActB[1],yProc[1]};
        ...
        {/* right side */
            ...
            {
                long fpixel[2]={xOverA[0],yProc[0]};
                long lpixel[2]={xOverA[1],yProc[1]};
                computeOvscMean(infptr, fpixel, lpixel, ovrscMeanL);
            }
            
            {
                long fpixel[2]={xActA[0],yProc[0]};
                long lpixel[2]={xActA[1],yProc[1]};
