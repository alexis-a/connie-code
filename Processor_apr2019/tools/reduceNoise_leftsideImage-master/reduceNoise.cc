...
int flipImage(dataType *inArray,const long nCol, const long nRow){
    dataType *tempArray = new dataType[nCol*nRow];
    
    for (long i=0; i<nRow; i++) {
        for (long j=0; j<nCol; j++) {
            tempArray[i*nCol + j] = inArray[i*nCol + j];
        }
    }
    ...
    for (long i=0; i<nRow; i++) {
        for (long j=0; j<nCol; j++) {
            inArray[i*nCol + j] = tempArray[i*nCol +nCol - 1 - j];
        }
    }
...
int computeImage(const char *inFile, const char *outFile, vector<int> &outHdu){
    ...
    for(unsigned int eI=0; eI<nUseHdu; ++eI)  /* Main loop through each extension */
    {
        ...
        //flip image horizontaly to reduce noise on left side
        int flipStatus = flipImage(inArray,naxes[0],naxes[1]);
        if (flipStatus != 0) return(flipStatus);
    ...
    for (unsigned int oi=0; oi<nOutHdu; ++oi)  /* Main loop through each extension */
    {
        ...
        /*flip output image to leave it in the original way*/
        int flipStatus = flipImage(outArray,naxesOut[0],naxesOut[1]);
        if (flipStatus != 0) return(flipStatus);
